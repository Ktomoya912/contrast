...
class ContrastGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.ai_model_path = "contrast_ac.pth"  # デフォルトモデル
    ...
    def create_widgets(self):
        ...
        info_frame = tk.LabelFrame(
            left_frame, text="ゲーム情報", padx=10, pady=10, bg="#FFFFFF"
        )
        info_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))

        self.current_player_label = tk.Label(
            info_frame, text="", font=("Arial", 12, "bold"), bg="#FFFFFF"
        )
        self.current_player_label.pack(pady=5)

        # タイル情報
        tile_frame = tk.LabelFrame(
            left_frame, text="タイル残数", padx=10, pady=10, bg="#FFFFFF"
        )
        tile_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))

        self.p1_black_label = tk.Label(
            tile_frame, text="", font=("Arial", 10), bg="#FFFFFF"
        )
        self.p1_black_label.pack(anchor=tk.W)

        self.p1_gray_label = tk.Label(
            tile_frame, text="", font=("Arial", 10), bg="#FFFFFF"
        )
        ...
        self.p2_black_label = tk.Label(
            tile_frame, text="", font=("Arial", 10), bg="#FFFFFF"
        )
        self.p2_black_label.pack(anchor=tk.W)

        self.p2_gray_label = tk.Label(
            tile_frame, text="", font=("Arial", 10), bg="#FFFFFF"
        )
        self.p2_gray_label.pack(anchor=tk.W)

        # タイル選択ボタン
        tile_select_frame = tk.LabelFrame(
            left_frame, text="タイル配置", padx=10, pady=10, bg="#FFFFFF"
        )
        ...
        help_frame = tk.LabelFrame(
            left_frame, text="操作方法", padx=10, pady=10, bg="#FFFFFF"
        )
...
3. タイルを配置する場合は
   ...
   （配置は任意）
        ...
        self.ai_toggle_btn = tk.Button(
            control_frame, text="AI有効化", command=self.toggle_ai, width=15, height=2
        )
        ...
        for y in range(self.board_size):
            for x in range(self.board_size):
                ...
                self.canvas.create_rectangle(
                    x1, y1, x2, y2, fill=fill, outline=self.colors["grid"], width=2
                )
    ...
    def select_piece(self, x: int, y: int):
        ...
        self.status_label.config(
            text=f"移動先を選択してください（移動可能: {len(self.valid_moves)}箇所）"
        )
    ...
    def cancel_tile_placement(self):
        ...
        self.status_label.config(
            text=f"Player {self.game.current_player.value} のターン"
        )
        self.update_tile_buttons()
        self.update_display()
    ...
    def ai_move(self):
        ...
        try:
            action_result = self.ai_learner.select_action(self.game)
    ...
    def update_display(self):
        ...
        self.current_player_label.config(
            text=f"Player {current} のターン", fg=self.colors[f"player{current}"]
        )
    ...
    def load_ai(self):
        ...
        tk.Label(
            dialog, text="AIがプレイするプレイヤーを選択", font=("Arial", 11)
        ).pack(pady=15)
        ...
        tk.Label(dialog, text="", height=1).pack()

        # モデルパス選択
        model_frame = tk.Frame(dialog)
        model_frame.pack(pady=5)

        tk.Label(model_frame, text="モデル:").pack(side=tk.LEFT, padx=5)
        model_var = tk.StringVar(value="contrast_ac.pth")
        model_entry = tk.Entry(model_frame, textvariable=model_var, width=20)
        ...
        def confirm():
            ...
            try:
                # Actor-Critic学習器を初期化
                self.ai_learner = ActorCriticLearner(
                    board_size=5,
                    max_actions=500,
                ...
                # モデルを読み込み
                try:
                    self.ai_learner.network.load(model_path)
                    status_msg = f"学習済みAIを読み込みました: {model_path}"
                except FileNotFoundError:
                    status_msg = (
                        f"警告: {model_path} が見つかりません。未学習のAIを使用します"
                    )
                    logger.warning(f"Model not found: {model_path}")
                except Exception as e:
                    status_msg = "警告: モデル読み込みエラー。未学習のAIを使用します"
                ...
                messagebox.showinfo("AI読み込み完了", status_msg)
        ...
        tk.Button(dialog, text="OK", command=confirm, width=10).pack(pady=15)
