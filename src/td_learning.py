...
class TDLearner:
    ...
    def select_action(
        self, game: ContrastGame, use_epsilon: bool = True
    ) -> Optional[Tuple]:
        """
        行動選択（ε-greedy）
    ...
    ) -> Tuple[Player, List[Tuple[np.ndarray, float]]]:
        """
        1ゲームをプレイ
    ...
    def train(
        ...
        model_path: str = "contrast_ai.pth",
    ):
        """
        複数エピソードの学習
...
if __name__ == "__main__":
    ...
    # 少数のエピソードで学習テスト
    learner.train(
        num_episodes=1000, save_interval=100, model_path="contrast_ai_demo.pth"
    )
